...
tabela_itens_mh <- tabela_itens_mh %>% 
  ...
  dplyr::select(-'N categoria 1', -'N categoria 2',-'N categoria 3',-'N categoria 4',-'N categoria 5')
...
v_mh2 <- bd_mh_fake %>% 
  dplyr::select(Mtl_Hth_01, Mtl_Hth_05, Mtl_Hth_06, Mtl_Hth_09:Mtl_Hth_12) %>% 
  colnames()
...
# Verificar resultados de model fit
fitMeasures(efa_bfi2)

# Antes de seguir em frente, é melhor inverter os escores
bd_mh_fake <- bd_mh_fake %>%
  mutate_at(vars(Mtl_Hth_05, Mtl_Hth_06, Mtl_Hth_09, Mtl_Hth_10, Mtl_Hth_11),
            funs(case_when(
              . == "1"  ~ 5,
              . == "2"  ~ 4,
              . == "3"  ~ 3,
...
# Confiabilidade
psych::alpha(x = bd_mh_fake[,v_mh2], check.keys = TRUE) # 0.79

jmv::reliability(data = bd_mh_fake, 
                 vars = c("Mtl_Hth_01", "Mtl_Hth_05", "Mtl_Hth_06", "Mtl_Hth_09", "Mtl_Hth_10", "Mtl_Hth_11", "Mtl_Hth_12"), 
                 #revItems = c("Mtl_Hth_01", "Mtl_Hth_12"),  
                 omegaScale = TRUE,  
                 alphaItems = TRUE,  
                 omegaItems = TRUE)
...
# Use a função scoreScale do pacote PROscorerTools para calcular o POMP score
bd_mh_fake$mh_pompscore <- scoreScale(
  bd_mh_fake,
  items = v_mh2,
  #revitems = FALSE,
  minmax = c(1,6),
...
# Como a variável é gerada no formato de lista, é preciso convertê-la para vetor dentro do data.frame
bd_mh_fake$mh_pompscore <- as.vector(unlist(bd_mh_fake$mh_pompscore))
str(bd_mh_fake$mh_pompscore)

psych::describe(bd_mh_fake$mh_pompscore) # n = 2251, mean = 47.37 
